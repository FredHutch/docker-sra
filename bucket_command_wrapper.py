...
class BCW():
    ...
    def __init__(self):
        ...
        if not args.download_files:
            ...
            raw_download_files = [f.strip()
                                  for f in args.download_files if f.strip() != ""]
            if len(raw_download_files) > 0:
                self.download_files = self.parse_download_files(
                    raw_download_files)
            else:
                self.download_files = []
        ...
        if not args.upload_files:
            ...
            raw_upload_files = [f.strip()
                                for f in args.upload_files if f.strip() != ""]
    ...
    def upload_files_to_bucket(self):
        upload_files = self.upload_files + \
            [f for f in self.download_files if f['mode'] == 'rw']
